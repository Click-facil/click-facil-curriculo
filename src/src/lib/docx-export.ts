...
export const exportToDocx = async (data: ResumeData) => {
  ...
  // Contact
  const contactParts = [personalInfo.email, personalInfo.phone, `${personalInfo.city} - ${personalInfo.state}`].filter(Boolean);
  sections.push(
    new Paragraph({
  ...
  if (experience.length > 0) {
    sections.push(sectionTitle("EXPERIÊNCIA PROFISSIONAL"));
  ...
  if (education.length > 0) {
    sections.push(sectionTitle("FORMAÇÃO ACADÊMICA"));
  ...
  // Courses
  if (courses.length > 0) {
    sections.push(sectionTitle("CURSOS E CERTIFICAÇÕES"));
    courses.forEach((c) => {
      sections.push(new Paragraph({
        children: [
